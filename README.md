...
7. selectively map the remaining unported public API families to their pinned upstream handlers;
...
Public model APIs keep Rust authentication at the outer boundary. Core chat, model-list, embeddings, audio, image, video, Responses, Claude, Gemini, Codex, search, and web-fetch paths remain native. Compatibility mode delegates only the currently unported model-list alias, Ollama `/v1/api/chat` and Gemini `/v1beta/models` generation paths. These responses are visibly marked `x-9router-runtime: upstream-compat` and are not counted as native parity.
...
```bash
./scripts/full-stack-smoke.sh http://127.0.0.1:20130
./scripts/full-stack-smoke-v2.sh http://127.0.0.1:20130
```
