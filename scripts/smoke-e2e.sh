...
curl -fsS http://127.0.0.1:20130/api/health >"$TMP/health.json"
grep -q '"runtime":"rust"' "$TMP/health.json"
curl -fsS -D "$TMP/health-headers.txt" -o /dev/null http://127.0.0.1:20130/api/health
...
curl -fsS -b "$COOKIE" -H 'content-type: application/json' \
  ...
  http://127.0.0.1:20130/api/provider-nodes >"$TMP/node.json"
...
curl -fsS -H "Authorization: Bearer $API_KEY" -H 'content-type: application/json' \
  --data-binary @"$TMP/openai.json" http://127.0.0.1:20130/v1/chat/completions >"$TMP/openai-out.json"
...
curl -fsS -H "x-api-key: $API_KEY" -H 'anthropic-version: 2023-06-01' -H 'content-type: application/json' \
  --data-binary @"$TMP/claude.json" http://127.0.0.1:20130/v1/messages >"$TMP/claude-out.json"
...
PY
curl -fsS -H "Authorization: Bearer $API_KEY" -H 'content-type: application/json' \
  --data-binary @"$TMP/responses.json" http://127.0.0.1:20130/v1/responses >"$TMP/responses-out.json"
...
curl -fsS -N -H "Authorization: Bearer $API_KEY" -H 'content-type: application/json' \
  --data-binary @"$TMP/stream.json" http://127.0.0.1:20130/v1/chat/completions >"$TMP/stream-out.txt"
grep -q 'hello ' "$TMP/stream-out.txt"
grep -q 'from mock' "$TMP/stream-out.txt"
grep -q '\[DONE\]' "$TMP/stream-out.txt"

curl -fsS -H "Authorization: Bearer $API_KEY" http://127.0.0.1:20130/v1/models >"$TMP/models.json"
grep -q '"object":"list"' "$TMP/models.json"
curl -fsS -b "$COOKIE" 'http://127.0.0.1:20130/api/usage/stats?period=all' >"$TMP/usage.json"
